...
clone_tools() {
    ...
    PATCHESVER=$(grep version $CURDIR/revanced-patches/gradle.properties | cut -d = -f 2)
    CLIVER=$(grep version $CURDIR/revanced-cli/gradle.properties | cut -d = -f 2)
}
...
build_tools() {
    ...
    ls $CURDIR/revanced-patches/patches/build/libs/
